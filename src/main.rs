...
use places_autocomplete_rs::places_data_dir;
...
#[get("/search")]
async fn search(web::Query(info): web::Query<HashMap<String, String>>) -> impl Responder {
...
async fn main() -> std::io::Result<()> {
    let data_dir = places_data_dir();
    println!("Loading location data from {:?}", data_dir);
